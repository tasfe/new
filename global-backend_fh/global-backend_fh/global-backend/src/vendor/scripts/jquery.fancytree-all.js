...
$.ui.fancytree.registerExtension({
	...
	nodeSetStatus: function(ctx, status, message, details) {
		...
		if(node.parent){
			span = this.$div.find("span.fancytree-expander", node.span).get(0);
		}else{
			span = this.$div.find(".fancytree-statusnode-wait, .fancytree-statusnode-error", node[this.nodeContainerAttrName])
