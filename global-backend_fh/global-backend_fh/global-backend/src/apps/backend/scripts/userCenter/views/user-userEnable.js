define(function (require, exports, module) {
    ...
    var UserEnableView = Base.ItemView.extend({

        template: require('text!userCenter/templates/user-userEnable.html'),
            ...
            var a = self.getDisableRecordXhr({userId: self.options.userId}).done(function (res) {
                if (res && res.result === 0) {
                    if (_(res.root).size() > 0) {
                        ...
                        self.$('.js_ul_enable').html('<button type="button" class="btn btn-danger" data-toggle="collapse" data-target="#collapseExample" aria-expanded="false" aria-controls="collapseExample">' +
                            '点击查看记录</button>');
                        self.$('.js-ul-viewEnable').html('<div class="js-ul-showRecord collapse" id="collapseExample"><div class="well height-md overflow-auto">' +
                            '</div></div>');
