define(function (require, exports, module) {
  ...
  var DividendAuditView = require('agGame/amountManagement/ag-subDividManagement-audit');
  var fundStatusConfig = require('fundCenter/misc/fundStateTDConfig');

  var AgSubDividManagementView = Base.Prefab.SearchGrid.extend({

    template: require('text!agGame/amountManagement/ag-subDividManagement.html'),
    status_template: require('text!agGame/amountManagement/ag-StatusTD-Temp.html'),

    events: {
      'click .js-ag-sub-div-deal': 'dealWithdrawHandler',
      'click .js-ag-sub-div-notDeal': 'notDealWithdrawHandler',
      'click .js-fc-gm-account':'userAccountHandler',
      'change .js-ag-sub-dr': 'settleDateChangeHandler',
      'click .js-ag-sub-log':'checkLogHandler'
    },
    ...
    initialize: function () {
      _(this.options).extend({
        ...
        ajaxOps: {
          url: '/intra/agmanager/agdividlist.json'
        }
      });
    },
    ...
    getCheckUserListXhr: function(){
      return Global.sync.ajax({
        url: '/intra/agmanager/operator.json'
      });
    },
    getCheckSalesListXhr: function(){
      return Global.sync.ajax({
        url: '/intra/agmanager/merchantlist.json'
      });
    },
    dealDividendXhr: function(data){
      return Global.sync.ajax({
        url: '/intra/agmanager/approve.json',
        data:data
      });
    },
    ...
    onRender: function(){
      ...
      this.getCheckUserListXhr() .fail(function(){
        ...
        if(res.result===0){
          ...
          self.renderSelect(optionData,self.$('.js-ag-sub-operatorId'))
        }else{
          Global.ui.notification.show('操作失败。');
        }
      });
      this.getCheckSalesListXhr() .fail(function(){
      }).done(function(res){
        if(res.result===0){
          var optionData = _(res.root||[]).map(function(user){
            return {
              value: user.userId,
              text: user.userName
            }
          });
          self.renderSelect(optionData,self.$('.js-ag-sub-parentId'))
        }else{
          Global.ui.notification.show('操作失败。');
        }
      });
      this.generateSettlementOption(this.$('.js-ag-sub-dr'));
      this.$('.js-ag-sub-dr').trigger('change');
    ...
    renderGrid: function (gridData) {
      var rowsData = _(gridData.dataList).map(function (dividend, index) {
        return {
    ...
    formatRowData: function(dividend){
      ...
      switch(dividend.status){
        case 0: status='未发放';break;
        case 1: status='已发放';break;
        case 2: status='已拒绝';break;
        case 3: status='不发放';break;
      }
      ...
      if(dividend.status===2 || dividend.status===1){
        row.push( '<a class="js-ag-sub-log btn btn-link no-padding" >日志</a>');
      }else{
        row.push( '');
      }
    ...
    propDividendDealModel: function (type,data) {
      ...
      var $dialog = Global.ui.dialog.show(
        {
          title:  '提示',
          body: '<div class="js-sub-div-Check-container"></div>',
          footer: ''
        }
      );
      _(data).extend({
      ...
      var $checkDividendContainer = $dialog.find('.js-sub-div-Check-container');
      ...
      var $notice = $dialog.find('.js-ag-sub-deal-notice');
      $dialog.off('click.saveInfo')
        .on('click.saveInfo', '.js-ag-sub-audit-submit', function (ev) {
          var $target = $(ev.currentTarget);
          $target.button('loading');
          var clpValidate = $dialog.find('.js-ag-sub-deal-form').parsley().validate();
          if (clpValidate) {
            ...
            var data2 = {
              ...
              remark: $dialog.find('.js-ag-sub-deal-remark').val()
            };
    ...
    userAccountHandler:function(e){
      ...
      Global.appRouter.navigate(_('#sc/sbl').addHrefArgs({
        _t:_.now(),
  ...
  module.exports = AgSubDividManagementView;
});