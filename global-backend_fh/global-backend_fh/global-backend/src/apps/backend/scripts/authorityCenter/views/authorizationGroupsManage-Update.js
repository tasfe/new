define(function (require, exports, module) {
  ...
  var UpdateAuthorizationGroupView = Base.ItemView.extend({

    template: require('text!authorityCenter/templates/authorizationGroupsManage-Update.html'),
    ...
    onRender: function () {
      ...
      this.$groupId = this.$('.js-ac-cg-groupId');
      this.$groupName = this.$('.js-ac-update-groupName');
      this.$groupNameLan=this.$('.js-ac-cg-groupName');
      this.$tree2 = this.$("#updateTree2");
      this.$tree1 = this.$("#updateTree1");
      this.$switch = this.$('.js-ac-uag-switch');
      ...
      this.$groupNameLan.html('<label class="control-label m-right-sm">组名称：</label><input class="js-ac-update-groupName" data-parsley-zhAndLetNumMaxLen="20" value="'+groupName+'" required>');
      this.$groupId.val(groupId);
      this.$groupName.val(groupName);

      //1 初始化左侧树形菜单
      var source;
      if (_(groupId).isNull() || parentGroupId === '0') {
        source = {
    ...
    saveGroupInfoHandler: function (e) {
      ...
      var $form = this.$('.js-ac-agm-update-form');
    ...
    insertNotice: function ( html) {
      this.$('.js-ac-ug-notice').html(this._getErrorMsg(html));
    },
    //组装错误提示框
  ...
  module.exports = UpdateAuthorizationGroupView;
});
