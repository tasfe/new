...
//        ,abort: false
  ...
  renderGrid: function(gridData) {
    ...
    this.grid.addFooterRows({
        ...
        columnEls: [
          ...
          {
            colspan: 3,
            content: '<div><button class="js-ac-sm-pay-all font-sm ac-sm-pay-all" data-amount="'+ _(gridData.salaryAmountTotal).convert2yuan()+'">一键全派发</button></div>'
          }
        ]
      })
      .hideLoading();
  ...
  formatRowData: function(rowInfo) {
    ...
    switch (rowInfo.status) {
      case 0:
        statusDes = '未发放';
        operate = '<a class="js-ac-daily-salary-payout btn btn-link no-p-top no-p-bottom no-margin" data-id="' + rowInfo.salaryId + '">发放</a>';
        break;
      case 1:
        statusDes = '已发放';
        break;
  ...
  //  operate = operate + '<a class="js-ac-sm-set btn btn-link" data-name="'+rowInfo.userName+'" data-id="'+rowInfo.userId+'">设置</a>'
    row.push(statusDes);
    row.push(_(rowInfo.givenTime).toDate())
    row.push(operate);
    return row;
  },
  ...
  payoutSalaryHandler: function(e) {
    ...
    var salary = _($tr.data('salaryAmount')).convert2yuan();
    var salaryId = $target.data('id');
    $(document).confirm({
      title: '提示',
      content: '<div class="text-center"><div>确定将日薪发放至下级？</div><div><div class="margin-md inline-block">用户名：'+username+'</div><div class="margin-md  inline-block">日薪：'+salary+'</div></div></div>',
      agreeCallback: function() {
        $target.button('loading');
        self.payoutSalaryXhr({salaryId: salaryId}).done(function(res) {
          $target.button('reset');
          if(res.result===0){
            self.render();
            Global.ui.notification.show('操作成功！');
          }else if (res.result===1){
            Global.ui.notification.show(res.msg+',请充值!');
          }else{
  ...
  payoutAllHandler: function(e){
    ...
    //var $tr = $target.closest('tr');
    //var username = $tr.data('userName');
    //var salary = _($tr.data('saleAmount')).convert2yuan();
    //var salaryId = $target.data('id');
    $(document).confirm({
      title: '提示',
      content: '<div class="text-center"><div>是否发放所有用户薪资？</div></div>',//<div><div class="margin-md  inline-block">日薪总计：'+amount+'</div></div>
      agreeCallback: function() {
        $target.button('loading');
        self.payoutSalaryXhr({salaryId: -1}).done(function(res) {
          $target.button('reset');
          if(res.result===0){
            self.render();
            Global.ui.notification.show('操作成功！');
